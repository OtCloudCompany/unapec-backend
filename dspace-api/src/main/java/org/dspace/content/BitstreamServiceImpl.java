...
public class BitstreamServiceImpl extends DSpaceObjectServiceImpl<Bitstream> implements BitstreamService {
    ...
    public Bitstream create(Context context, Bundle bundle, InputStream is)
        ...
        UUID itemUUID = getItem(b).map(Item::getID).orElse(null);
        context.addEvent(
    ...
    public void update(Context context, Bitstream bitstream) throws SQLException, AuthorizeException {
        ...
        if (bitstream.isMetadataModified()) {
            UUID itemUUID = getItem(bitstream).map(Item::getID).orElse(null);
            context.addEvent(
    ...
    /**
     * Find the item a bitstream belongs to, so that audit events on a bitstream can be related to
     * the item that owns it.
     *
     * @param bitstream the bitstream to resolve
     * @return the owning item, or empty when the bitstream is not in any item's bundle
     * @throws SQLException if the bundles cannot be loaded
     */
